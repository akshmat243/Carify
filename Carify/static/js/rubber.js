document.addEventListener('DOMContentLoaded', function () {
  const areas = JSON.parse(document.getElementById('rubber-areas-data').textContent);
  ...
  document.querySelectorAll('#rubber-body tr').forEach(bindRow);

  document.getElementById('Critical-Rubber1').addEventListener('click', function () {
    ...
    `;
    document.getElementById('rubber-body').appendChild(row);
    bindRow(row);
  });
});
